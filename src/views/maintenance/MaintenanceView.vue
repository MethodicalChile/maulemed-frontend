...
const { canManageCatalogs, canCreateProducts, canDeleteProducts } =
  usePermissions();
...
<template>
  <section class="p-6 space-y-6">
    ...
    >
      <button
        v-if="canCreateProducts && activeTab === 'categories'"
      ...
      <button
        v-if="canCreateProducts && activeTab === 'units'"
        class="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-bold bg-primary text-primary-foreground rounded-xl shadow-md hover:bg-primary/90 transition-all hover:scale-105"
    ...
    >
          ...
          <template #actions="{ row }">
            <div class="flex justify-end gap-1">
              <button
                ...
                type="button"
                class="p-2 hover:bg-muted rounded-lg text-muted-foreground hover:text-primary transition-colors"
                title="Editar"
              ...
              <button
                ...
                type="button"
                class="p-2 hover:bg-muted rounded-lg text-muted-foreground hover:text-destructive transition-colors"
                title="Eliminar"
                @click="requestDelete('category', row)"
              >
                <Trash2 :size="16" />
              </button>
            </div>
      ...
      <template v-if="activeTab === 'units'">
        ...
        >
          <template #code="{ row }"
            ><span class="font-mono text-xs px-2 py-1 bg-muted rounded">{{
              row.code
            }}</span></template
          >
          <template #is_active="{ row }">
    ...
    >
      <form @submit.prevent="saveUnit">
        ...
        <div class="space-y-4">
          ...
          <label class="flex items-center gap-2 text-sm"
            ><input v-model="unitForm.is_active" type="checkbox" /> Unidad
            activa</label
          >
        </div>
        <div class="flex justify-end gap-3 mt-6 pt-4 border-t">
