...
function toggleModule(moduleKey) {
  if (minimizedModules.value.includes(moduleKey)) {
    minimizedModules.value = minimizedModules.value.filter(
      (k) => k !== moduleKey,
    );
  } else {
...
const {
  ...
  fill: roleFill,
  submit: roleSubmit,
} = useForm({ code: "", name: "", description: "", is_active: true }, (data) =>
  editingRole.value
...
async function loadMatrix() {
  ...
  try {
    ...
    allRoleCodes.value = rawRoles.map((r) =>
      typeof r === "string" ? r : r.code,
    );
    matrixModules.value = d.matrix ?? [];
  } catch (err) {
    matrixError.value =
      err.response?.data?.message ?? "Error al cargar la matriz.";
  } finally {
...
function permCount(roleCode) {
  ...
  matrixModules.value.forEach((m) =>
    m.permissions.forEach((p) => {
      if (p.roles?.includes(roleCode)) n++;
    }),
  );
  return n;
}
...
const canCreateRole = computed(
  () =>
    hasFullAccess.value ||
    Boolean(authStore.permissions?.can_create_roles),
);

const canEditRole = computed(
  () =>
    hasFullAccess.value ||
    Boolean(authStore.permissions?.can_edit_roles),
);

const canDeleteRole = computed(
  () =>
    hasFullAccess.value ||
    Boolean(authStore.permissions?.can_delete_roles),
);
...
async function togglePerm(permKey, roleCode) {
  ...
  if (savingPerm.value === key) return;
  ...
  try {
    ...
    for (const mod of matrixModules.value) {
      ...
      if (perm) {
        if (!wasGranted) {
          if (!perm.roles.includes(roleCode)) {
            perm.roles.push(roleCode);
          }
        } else {
    ...
    matrixModules.value = [...matrixModules.value];
...
<template>
  <section class="space-y-6">
    <PageHeader
      title="Roles"
      subtitle="Crea y gestiona los roles del sistema. Define quién puede hacer qué."
    >
      <button
        v-if="activeTab === 'list' && canCreateRole"
    ...
    <template v-if="activeTab === 'list'">
      ...
      <!-- Estado vacío -->
      <div
        v-if="!loadingRoles && !roles.length"
        class="text-center py-16 text-muted-foreground space-y-4"
      >
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div
          ...
          :key="role.uuid"
          :class="[
            'flex items-start gap-4 p-5 border rounded-lg bg-card shadow-sm transition-shadow',
            !role.is_active && 'opacity-60 bg-muted/50',
          ]"
        >
          <!-- Avatar del rol -->
          <div
            class="w-10 h-10 flex-shrink-0 rounded-xl bg-primary text-primary-foreground flex items-center justify-center font-extrabold text-lg"
          >
          ...
          <div class="flex flex-col items-end gap-2">
            <div class="flex flex-col items-end gap-1">
              ...
              <span
                class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-100 text-blue-800"
                >{{ permCount(role.code) }} accesos</span
              >
            </div>
            <div
    ...
    <template v-if="activeTab === 'matrix'">
      <div class="flex flex-wrap items-center gap-4 mb-6">
        <div class="flex items-center gap-2 px-3 py-2 border rounded-md">
          ...
          <input
            ...
            type="text"
            placeholder="Filtrar columnas por rol..."
        ...
        <div class="flex items-center gap-4 text-xs text-muted-foreground">
          ...
          <div class="flex items-center gap-1">
            <span class="w-2 h-2 rounded-full bg-primary"></span> Creado en BD
          </div>
        </div>
      </div>

      <div v-if="loadingMatrix" class="text-center py-12 text-muted-foreground">
        ...
        <table class="w-full text-sm border-collapse">
          <thead>
            <tr class="bg-muted/50 text-left">
              <th
                class="p-3 border-b font-semibold text-xs uppercase text-muted-foreground whitespace-nowrap min-w-[200px]"
              >
                Módulo / Acción
              </th>
              <th
          ...
          <tbody class="divide-y">
            <template v-for="mod in matrixModules" :key="mod.key">
              <tr
                class="bg-primary/5 border-t border-border cursor-pointer"
                @click="toggleModule(mod.key)"
              >
                <td
                  :colspan="visibleCodes.length + 1"
                  class="px-4 py-3 text-xs font-bold uppercase tracking-widest text-primary flex items-center gap-2"
                >
                  <component
                    :is="isMinimized(mod.key) ? ChevronRight : ChevronDown"
                    :size="14"
                  />
                  {{ mod.module }}
                </td>
              ...
              >
                ...
                <td
                  ...
                  :class="[
                    canEditRole && existingCodes.has(code)
                  ...
                  :title="
                    canEditRole
                      ? existingCodes.has(code)
                        ? hasPerm(perm.key, code)
                          ? 'Clic para quitar acceso'
                          : 'Clic para dar acceso'
                  ...
                  @click="
                    canEditRole &&
                    existingCodes.has(code) &&
                    togglePerm(perm.key, code)
                  "
                >
                  <div class="flex justify-center items-center h-full">
                    <span
                      v-if="isSaving(perm.key, code)"
                      class="w-4 h-4 border-2 border-primary border-t-transparent rounded-full animate-spin"
                    ></span>

                    <span
                      v-else-if="hasPerm(perm.key, code)"
                      class="w-5 h-5 flex items-center justify-center rounded-full text-green-600 font-bold"
                    >
                      <Check :size="16" stroke-width="3" />
                    </span>
                    ...
                    <span
                      v-else
                      class="w-5 h-5 flex items-center justify-center rounded-full text-red-500 font-bold"
                    >
                      <X :size="16" stroke-width="3" />
                    </span>
                  </div>
      ...
      <p class="text-xs text-muted-foreground pt-4">
        <span class="inline-block w-2 h-2 rounded-full bg-primary"></span> Roles
        marcados están creados en la BD y asignables a usuarios.
      </p>
    </template>
    ...
    >
      <form class="grid grid-cols-1 gap-4" @submit.prevent="handleRoleSubmit">
        ...
        <p v-if="!editingRole" class="text-xs text-muted-foreground -mt-3">
          Se genera automáticamente desde el nombre. Solo mayúsculas y guiones
          bajos.
        </p>
        ...
        >
          <p
            class="text-sm font-semibold text-foreground mb-2 flex items-center justify-between"
          >
            Accesos del sistema
            <span
              class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-100 text-blue-800"
              >{{ permCount(roleForm.code) }} accesos</span
            >
          </p>
          <div class="grid gap-1 max-h-40 overflow-y-auto text-xs">
            <template v-for="mod in matrixModules" :key="mod.key">
              <div
                v-for="perm in mod.permissions.filter((p) =>
                  p.roles?.includes(roleForm.code),
                )"
                :key="perm.key"
              ...
              >
                ...
                />
                <span
                  ><strong>{{ mod.module }}</strong> — {{ perm.action }}</span
                >
              </div>
            </template>
          </div>
