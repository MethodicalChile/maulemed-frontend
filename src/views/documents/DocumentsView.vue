...
<template>
  <section class="space-y-6">
    <PageHeader
      title="Documentos"
      subtitle="PDFs, facturas, guías de despacho y comprobantes"
    >
      <button
    ...
    <div class="flex items-center gap-4">
      ...
      <AppSelect
        class="text-sm"
        :model-value="docList.params.document_type"
        @update:model-value="docList.setParam('document_type', $event)"
      >
      ...
      <AppSelect
        class="text-sm"
        :model-value="docList.params.related_model"
        @update:model-value="docList.setParam('related_model', $event)"
      >
        <option value="">Todos los módulos</option>
        <option
          v-for="m in RELATED_MODELS.filter((m) => m.value !== '')"
          :key="m.value"
          :value="m.value"
        >
          {{ m.label }}
    ...
    <AppAlert
      v-if="docList.error.value || deleteError"
      type="error"
      :message="docList.error.value || deleteError"
    />

    <AppTable
      :columns="columns"
      :rows="docList.items.value"
      :loading="docList.loading.value"
    >
      <template #document_type="{ row }">
        <span
          class="text-[0.78rem] font-semibold px-2 py-1 rounded-full border border-border bg-muted whitespace-nowrap"
          >{{ typeLabel(row.document_type) }}</span
        >
      </template>
      <template #file_name="{ row }">
        <div class="flex items-center gap-1.5 text-sm">
          ...
          <span v-if="row.file_size" class="text-xs text-muted-foreground">{{
            fileSize(row.file_size)
          }}</span>
        </div>
      </template>
      <template #related_model="{ row }">{{
        modelLabel(row.related_model)
      }}</template>
      <template #uploaded_by="{ row }">{{
        row.uploaded_by_detail?.full_name ?? "—"
      }}</template>
    ...
    >
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <FormField label="Archivo" required class="col-span-full">
          <div
            class="flex flex-col items-center gap-1.5 p-7 border-2 border-dashed border-border rounded-lg cursor-pointer transition-colors bg-slate-50 text-center hover:border-primary"
