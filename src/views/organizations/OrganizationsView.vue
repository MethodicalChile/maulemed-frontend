...
const {
  ...
  fill: orgFill,
  submit: orgSubmit,
} = useForm({ name: "", rut: "", description: "", is_active: true }, (data) =>
  editingOrg.value
...
async function confirmDeleteOrg() {
  ...
  try {
    await organizationsApi.deleteOrganization(deleteOrgTarget.value.uuid);
...
async function confirmDeleteBranch() {
  ...
  try {
    await organizationsApi.deleteBranch(deleteBranchTarget.value.uuid);
...
function openCreateLe() {
  editingLe.value = null;
  leReset();
...
async function confirmDeleteLe() {
  ...
  try {
    await organizationsApi.deleteLegalEntity(deleteLe.value.uuid);
    deleteLe.value = null;
    leList.load();
  } finally {
    deleteLeLoading.value = false;
  }
}
...
async function confirmDeleteCc() {
  ...
  try {
    await organizationsApi.deleteCostCenter(deleteCc.value.uuid);
    deleteCc.value = null;
    ccList.load();
  } finally {
    deleteCcLoading.value = false;
  }
}
...
<template>
  <section class="page">
    <PageHeader
      title="Organización"
      subtitle="Gestión de organizaciones, sucursales, entidades legales y centros de costo"
    >
      <button
        v-if="
          permissions.can('can_create_organizations') &&
          activeTab === 'Organizaciones'
        "
        class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold bg-primary text-primary-foreground rounded-md hover:bg-primary/90"
        @click="openCreateOrg"
      >
    ...
    <template v-if="activeTab === 'Organizaciones'">
      <div
        v-if="permissions.can('can_create_organizations')"
      ...
      >
        <div class="relative flex-1">
          ...
          <AppInput
            ...
            :model-value="orgList.params.search"
            class="pl-10"
            @update:model-value="orgList.setParam('search', $event)"
          />
        </div>
      </div>
      ...
      <AppAlert
        v-if="orgList.error.value"
        type="error"
        :message="orgList.error.value"
      />
      <AppTable
        :columns="orgCols"
        :rows="orgList.items.value"
      ...
      >
        <template #is_active="{ row }">
          <span
            ...
            >{{ row.is_active ? "Activo" : "Inactivo" }}</span
          >
        </template>
        <template #actions="{ row }">
          <button
            v-if="canEditRow(row) || canDeleteRow(row)"
      ...
      <AppPagination
        :count="orgList.pagination.count"
        :page="orgList.pagination.page"
        :page-size="orgList.pagination.pageSize"
        @change="orgList.setPage"
      />
    </template>
    ...
    <template v-if="activeTab === 'Sucursales'">
      <div
        v-if="permissions.can('can_create_organizations')"
        class="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 rounded-xl bg-card border border-border shadow-sm mb-4"
      >
        <div class="relative col-span-2">
          ...
          <AppInput
            ...
            :model-value="branchList.params.search"
            class="pl-10"
            @update:model-value="branchList.setParam('search', $event)"
          />
        </div>
      </div>
      <div v-else class="mb-4">
      ...
      <AppAlert
        v-if="branchList.error.value"
        type="error"
        :message="branchList.error.value"
      />
      <AppTable
        :columns="branchCols"
        :rows="branchList.items.value"
        :loading="branchList.loading.value"
      >
        <template #organization="{ row }">{{
          row.organization_detail?.name ?? "—"
        }}</template>
      ...
      <AppPagination
        :count="branchList.pagination.count"
        :page="branchList.pagination.page"
        :page-size="branchList.pagination.pageSize"
        @change="branchList.setPage"
      />
    </template>
    ...
    <template v-if="activeTab === 'Entidades Legales'">
      ...
      >
        <div class="relative col-span-2">
          ...
          <AppInput
            ...
            :model-value="leList.params.search"
            class="pl-10"
            @update:model-value="leList.setParam('search', $event)"
          />
        </div>
      </div>
      <div v-else class="mb-4">
      ...
      <AppAlert
        v-if="leList.error.value"
        type="error"
        :message="leList.error.value"
      />
      <AppTable
        :columns="leCols"
        :rows="leList.items.value"
        :loading="leList.loading.value"
      >
        <template #organization="{ row }">{{
          row.organization_detail?.name ?? "—"
        }}</template>
      ...
      <AppPagination
        :count="leList.pagination.count"
        :page="leList.pagination.page"
        :page-size="leList.pagination.pageSize"
        @change="leList.setPage"
      />
    </template>
    ...
    <template v-if="activeTab === 'Centros de Costo'">
      ...
      >
        <div class="relative col-span-2">
          ...
          <AppInput
            ...
            :model-value="ccList.params.search"
            class="pl-10"
            @update:model-value="ccList.setParam('search', $event)"
          />
        </div>
      </div>
      <div v-else class="mb-4">
      ...
      <AppAlert
        v-if="ccList.error.value"
        type="error"
        :message="ccList.error.value"
      />
      <AppTable
        :columns="ccCols"
        :rows="ccList.items.value"
        :loading="ccList.loading.value"
      >
        <template #legal_entity="{ row }">{{
          row.legal_entity_detail?.name ?? "—"
        }}</template>
      ...
      <AppPagination
        :count="ccList.pagination.count"
        :page="ccList.pagination.page"
        :page-size="ccList.pagination.pageSize"
        @change="ccList.setPage"
      />
    </template>
    ...
    >
      <form class="grid grid-cols-1 gap-4" @submit.prevent="handleOrgSubmit">
        ...
        <FormField label="Descripción"
          ><AppTextarea v-model="orgForm.description" rows="2"
        /></FormField>
        <label class="flex items-center gap-2 text-sm"
          ><input v-model="orgForm.is_active" type="checkbox" /> Activo</label
        >
    ...
    <AppModal
      ...
      size="lg"
      @close="showBranchForm = false"
    >
      <form
        class="grid grid-cols-1 md:grid-cols-2 gap-4"
        @submit.prevent="handleBranchSubmit"
      >
        ...
        <FormField label="Entidad legal">
          <AppSelect v-model="branchForm.legal_entity">
            ...
            <option
              v-for="le in legalEntityOptions"
              :key="le.id"
              :value="le.id"
            >
              {{ le.name }}
        ...
        <div class="col-span-full flex gap-4">
          <label class="flex items-center gap-2 text-sm"
            ><input v-model="branchForm.is_main_branch" type="checkbox" /> Es
            sucursal principal</label
          >
          <label class="flex items-center gap-2 text-sm"
            ><input v-model="branchForm.is_active" type="checkbox" />
            Activo</label
          >
        </div>
        <div class="col-span-full flex justify-end gap-3 pt-4 border-t">
          ...
          >
            {{ branchLoading ? "Guardando..." : "Guardar" }}
          </button>
        </div>
      </form>
    </AppModal>
    ...
    <AppModal
      v-if="showLeForm"
      :title="editingLe ? 'Editar entidad legal' : 'Nueva entidad legal'"
      size="md"
      @close="showLeForm = false"
    >
      <form class="grid grid-cols-1 gap-4" @submit.prevent="handleLeSubmit">
        ...
        <FormField label="Razón social" required>
          <AppInput
            ...
            type="text"
            required
            placeholder="Ej: Clínica MauleMed SpA"
          />
        </FormField>
        ...
        <FormField label="Giro comercial">
          <AppInput
            v-model="leForm.business_activity"
            type="text"
        ...
        <label class="flex items-center gap-2 text-sm"
          ><input v-model="leForm.is_active" type="checkbox" /> Activo</label
        >
        <div class="flex justify-end gap-3 pt-4 border-t">
          <button
            type="button"
            class="px-4 py-2 text-sm font-medium border rounded hover:bg-muted"
            @click="showLeForm = false"
          >
            Cancelar
          </button>
          <button
    ...
    <AppModal
      v-if="showCcForm"
      :title="editingCc ? 'Editar centro de costo' : 'Nuevo centro de costo'"
      size="md"
      @close="showCcForm = false"
    >
      <form class="grid grid-cols-1 gap-4" @submit.prevent="handleCcSubmit">
        ...
        <label class="flex items-center gap-2 text-sm"
          ><input v-model="ccForm.is_active" type="checkbox" /> Activo</label
        >
        <div class="flex justify-end gap-3 pt-4 border-t">
          <button
            type="button"
            class="px-4 py-2 text-sm font-medium border rounded hover:bg-muted"
            @click="showCcForm = false"
          >
            Cancelar
          </button>
          <button
    ...
    >
      <div class="grid gap-2">
        <button
          ...
          @click="
            activeActionRow.type === 'org'
              ? openEditOrg(activeActionRow)
              : activeActionRow.type === 'branch'
                ? openEditBranch(activeActionRow)
                : activeActionRow.type === 'le'
                  ? openEditLe(activeActionRow)
                  : openEditCc(activeActionRow);
        ...
        <button
          ...
          @click="
            activeActionRow.type === 'org'
              ? (deleteOrgTarget = activeActionRow)
              : activeActionRow.type === 'branch'
                ? (deleteBranchTarget = activeActionRow)
                : activeActionRow.type === 'le'
                  ? (deleteLe = activeActionRow)
                  : (deleteCc = activeActionRow);
            showActionModal = false;
          "
        >
    ...
    <!-- ══ CONFIRMACIONES ══ -->
    <ConfirmDialog
      v-if="deleteOrgTarget"
      title="Eliminar organización"
      :message="`¿Eliminar &quot;${deleteOrgTarget.name}&quot;?`"
      confirm-label="Eliminar"
      :loading="deleteOrgLoading"
      @confirm="confirmDeleteOrg"
      @cancel="deleteOrgTarget = null"
    />
    <ConfirmDialog
      v-if="deleteBranchTarget"
      title="Eliminar sucursal"
      :message="`¿Eliminar &quot;${deleteBranchTarget.name}&quot;?`"
      confirm-label="Eliminar"
    ...
    <ConfirmDialog
      ...
      :message="`¿Eliminar &quot;${deleteLe.name}&quot;?`"
      confirm-label="Eliminar"
      :loading="deleteLeLoading"
      @confirm="confirmDeleteLe"
      @cancel="deleteLe = null"
    />
    <ConfirmDialog
      v-if="deleteCc"
      title="Eliminar centro de costo"
      :message="`¿Eliminar &quot;${deleteCc.name}&quot;?`"
      confirm-label="Eliminar"
      :loading="deleteCcLoading"
      @confirm="confirmDeleteCc"
      @cancel="deleteCc = null"
    />
  </section>
</template>
