...
const { items, loading, error, pagination, params, load, setPage, setParam } =
  useList(suppliersApi.listSuppliers);
...
async function submit(data) {
  if (editingItem.value) {
    return await suppliersApi.updateSupplier(editingItem.value.uuid, data);
  } else {
...
async function confirmDelete() {
  ...
  try {
    await suppliersApi.deleteSupplier(deleteTarget.value.uuid);
...
async function openProductsModal(supplier) {
  ...
  try {
    const [spRes, prRes] = await Promise.allSettled([
      ...
      suppliersApi.listSupplierProducts({
    ...
    spError.value = e.response?.data?.message ?? "Error al cargar productos";
  } finally {
...
function openEditSP(sp) {
  ...
  spForm.value = {
    ...
    allows_credit: sp.allows_credit,
    allows_cash_purchase: sp.allows_cash_purchase,
    is_active: sp.is_active,
  };
  showSPModal.value = true;
}
async function handleSPSubmit() {
  spFormLoading.value = true;
  spFormError.value = "";
  try {
    ...
    spFormError.value = e.response?.data?.message ?? "Error al guardar";
  } finally {
    spFormLoading.value = false;
  }
}
...
function productName(productId) {
  return (
    allProducts.value.find((p) => p.uuid === productId || p.id === productId)
      ?.name ?? productId
  );
}
...
async function openPrices(sp) {
  ...
  try {
    const res = await suppliersApi.listPrices({
      supplier_product: sp.uuid,
      page_size: 200,
    });
...
async function handlePriceSubmit() {
  priceFormLoad.value = true;
  priceFormError.value = "";
  try {
    const payload = {
      ...priceForm.value,
      supplier_product: viewingSP.value.uuid,
    };
    editingPrice.value
    ...
    priceFormError.value = e.response?.data?.message ?? "Error al guardar";
  } finally {
    priceFormLoad.value = false;
  }
}
...
<template>
  <section class="space-y-6">
    <PageHeader
      title="Proveedores"
      subtitle="Gestión de proveedores y sus precios"
    >
      <button
    ...
    <div
      class="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 rounded-xl bg-card border border-border shadow-sm hidden"
    >
      <div class="relative">
        <Search
          :size="18"
      ...
      <AppSelect
        :model-value="params.is_active"
        @update:model-value="setParam('is_active', $event)"
      >
    ...
    >
      <div class="grid gap-2">
        <button
          ...
          @click="
            openProductsModal(activeActionRow);
    ...
    >
      ...
      >
        ...
        <FormField label="Contacto" :error="errors.contact_name">
          <Field v-slot="{ field }" name="contact_name">
            <AppInput v-bind="field" type="text" />
          </Field>
        </FormField>
        ...
        <FormField label="Días de entrega" :error="errors.delivery_days">
          <Field v-slot="{ field }" name="delivery_days">
            <AppInput v-bind="field" type="number" />
          </Field>
        </FormField>
        <FormField
        ...
        >
          <Field v-slot="{ field }" name="payment_terms">
            <AppInput v-bind="field" type="text" />
          </Field>
        </FormField>
        <FormField
    ...
    >
      <div class="flex justify-between items-center mb-4">
        <span class="text-sm text-muted-foreground"
          >{{ spList.length }} producto(s) vinculado(s)</span
        >
        <button
          v-if="canCreateSuppliers"
          class="inline-flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-primary-foreground bg-primary rounded-md hover:bg-primary/90"
          @click="openCreateSP"
        >
      ...
      <div v-if="spLoading" class="text-center p-6 text-muted-foreground">
        Cargando...
      </div>
      <table v-else-if="spList.length" class="w-full text-sm border-collapse">
        <thead>
          <tr
            class="bg-muted/50 text-xs font-semibold uppercase text-muted-foreground"
          >
        ...
        <tbody class="divide-y">
          <tr v-for="sp in spList" :key="sp.uuid">
            <td class="p-2">
              {{ sp.product_detail?.name ?? productName(sp.product) }}
            </td>
            <td class="p-2">{{ sp.supplier_sku ?? "—" }}</td>
            <td class="p-2">
              {{ sp.last_price ? fmtCLP(sp.last_price) : "—" }}
            </td>
            ...
            <td class="p-2">
              <div class="flex gap-1">
                ...
                <button
                  v-if="canDeleteSuppliers"
                  class="p-1.5 rounded-md hover:bg-muted text-destructive hover:bg-destructive/10"
                  title="Desvincular"
                  @click="deleteSP(sp)"
                >
                  <XCircle :size="14" />
      ...
      <p
        v-else-if="!spLoading"
        class="text-muted-foreground text-sm text-center py-6"
      >
      ...
      <form class="grid grid-cols-1 gap-4" @submit.prevent="handleSPSubmit">
        ...
        <div class="flex flex-wrap gap-4 text-sm">
          <label class="flex items-center gap-2"
            ><input v-model="spForm.requires_purchase_order" type="checkbox" />
            Requiere OC</label
          >
          <label class="flex items-center gap-2"
            ><input v-model="spForm.allows_credit" type="checkbox" /> Permite
            crédito</label
          >
          <label class="flex items-center gap-2"
            ><input v-model="spForm.allows_cash_purchase" type="checkbox" />
            Permite compra en efectivo</label
          >
          <label class="flex items-center gap-2"
            ><input v-model="spForm.is_active" type="checkbox" /> Activo</label
          >
        </div>
    ...
    >
      <div class="flex justify-between items-center mb-4">
        <span class="text-sm text-muted-foreground"
          >{{ priceList.length }} registro(s)</span
        >
        <button
          v-if="canCreateSuppliers"
          class="inline-flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-primary-foreground bg-primary rounded-md hover:bg-primary/90"
      ...
      <div v-if="priceLoading" class="text-center p-6 text-muted-foreground">
        Cargando...
      </div>
      <table
        v-else-if="priceList.length"
      ...
      >
        <thead>
          ...
          >
            ...
            <th class="p-2 text-left">Moneda</th>
            <th class="p-2 text-left">Desde</th>
            <th class="p-2 text-left">Hasta</th>
        ...
        <tbody class="divide-y">
          <tr v-for="p in priceList" :key="p.uuid">
            ...
            <td class="p-2">
              {{ p.valid_to ? fmtDate(p.valid_to) : "Vigente" }}
            </td>
          ...
          <div class="grid grid-cols-1 gap-4">
            ...
            <FormField label="Fuente / referencia">
              <AppInput
                ...
                type="text"
                placeholder="Ej: Cotización, factura..."
              />
            </FormField>
          </div>
