...
const canCreateUser = computed(
  () =>
    hasFullAccess.value ||
    Boolean(authStore.permissions?.can_create_users),
);

const canEditUser = computed(
  () =>
    hasFullAccess.value ||
    Boolean(authStore.permissions?.can_edit_users),
);

const canDeleteUser = computed(
  () =>
    hasFullAccess.value ||
    Boolean(authStore.permissions?.can_delete_users),
);
...
async function loadOptions() {
  // Las opciones solo son necesarias si puede crear o editar usuarios
  if (!canCreateUser.value && !canEditUser.value) {
    allRoles.value = [];
  ...
  if (rolesRes.status === "fulfilled") {
    ...
    allRoles.value = Array.isArray(d)
      ? d
      : (d.results ?? d);
  }
  ...
  if (orgsRes.status === "fulfilled") {
    ...
    allOrganizations.value = Array.isArray(d)
      ? d
      : (d.results ?? d);
  }
  ...
  if (branchRes.status === "fulfilled") {
    ...
    allBranches.value = Array.isArray(d)
      ? d
      : (d.results ?? d);
  }
}
...
const visibleFormRoles = computed(() =>
  formRoleRows.value.filter((r) => !r._deleted),
);

function addFormRole() {
  formRoleRows.value.push(newFormRoleRow());
}
...
const onSubmit = handleSubmit(async (f) => {
  ...
  try {
    ...
    if (!editingUser.value) {
      ...
      if (profileUuid)
        await usersApi
          .updateProfile(profileUuid, profileData)
          .catch(() => null);
      else
        await usersApi
          .createProfile({ user: userId, ...profileData })
          .catch(() => null);
    }
...
async function handlePwdSubmit() {
  ...
  try {
    ...
    const d = err.response?.data;
    pwdFormError.value =
      d?.message ?? d?.detail ?? "Error al cambiar contraseña.";
  } finally {
...
async function addRoleAssignment() {
  ...
  if (!newRoleRow.value.role) {
    rolesError.value = "Selecciona un rol.";
    return;
  }
  ...
  try {
    ...
    const idx = userList.items.value.findIndex(
      (u) => u.id === rolesTarget.value.id,
    );
...
<template>
  <section class="space-y-6">
    ...
    <AppAlert
      v-if="userList.error.value"
      type="error"
      :message="userList.error.value"
    />

    <!-- Tabla -->
    <AppTable
      :columns="columns"
      :rows="userList.items.value"
    ...
    >
      ...
      <template #filter-is_active>
        <AppMultiSelect
          ...
          :model-value="
            userList.params.is_active ? [userList.params.is_active] : []
          "
          @update:model-value="
            userList.setParam('is_active', $event[0] || '');
            userList.load();
          "
        />
      </template>
      ...
      <template #full_name="{ row }">
        <div class="flex items-center gap-3">
          <div
            class="w-9 h-9 flex items-center justify-center rounded-full bg-primary text-white font-bold text-sm"
          >
      ...
      <template #roles="{ row }">
        <div class="flex flex-wrap gap-1">
          ...
          <span
            v-if="(row.role_assignments ?? []).length > 3"
            class="text-xs px-2 py-0.5 rounded-full bg-muted text-muted-foreground font-semibold"
          >
            +{{ row.role_assignments.length - 3 }}
          </span>
          <span
            v-if="!(row.role_assignments ?? []).length"
      ...
      <template #is_active="{ row }">
        <StatusBadge
          ...
          :map="{ ACTIVO: 'green', INACTIVO: 'neutral' }"
        />
      </template>

      <template #actions="{ row }">
        <button
          v-if="canEditUser || canDeleteUser"
  ...
  >
    <div class="grid gap-2">
      ...
      <button
        ...
        class="flex items-center gap-3 w-full p-3 rounded-lg hover:bg-muted text-sm font-medium"
        @click="
          toggleTarget = activeActionRow;
          showActionModal = false;
        "
      >
        <component
          :is="activeActionRow.is_active ? UserX : UserCheck"
          :size="16"
        />
  ...
  >
    <form @submit.prevent="onSubmit">
      ...
      <p
        class="text-xs font-bold uppercase tracking-wider text-muted-foreground border-b pb-1 mt-4 mb-3"
      >
        Datos de acceso
      </p>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <FormField label="Nombre de usuario" required :error="errors.username">
          <Field v-slot="{ field }" name="username">
            <AppInput v-bind="field" type="text" autocomplete="off" />
          </Field>
        </FormField>
        <FormField label="Email" :error="errors.email">
          <Field v-slot="{ field }" name="email">
            <AppInput v-bind="field" type="email" />
          </Field>
        </FormField>
        ...
        <template v-if="!editingUser">
          ...
          >
            <Field v-slot="{ field }" name="password_confirm">
              <AppInput
                v-bind="field"
                type="password"
                autocomplete="new-password"
      ...
      <p
        class="text-xs font-bold uppercase tracking-wider text-muted-foreground border-b pb-1 mt-6 mb-3"
      >
        Perfil
      </p>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <FormField label="RUT" :error="errors.rut">
          <Field v-slot="{ field }" name="rut">
            <AppInput v-bind="field" type="text" placeholder="12.345.678-9" />
          </Field>
        </FormField>
        ...
        >
          <Field v-slot="{ field }" name="position">
            <AppInput
              v-bind="field"
              type="text"
              placeholder="Ej: Jefa de Sucursal"
            />
          </Field>
        </FormField>
      </div>
      ...
      <div class="mt-6 pt-4 border-t">
        ...
        <div v-else class="space-y-2">
          <div
            v-for="r in visibleFormRoles"
            :key="r._key"
            class="flex gap-2 p-2 border rounded-md bg-muted/50 items-end"
          >
            <!-- Rol -->
            ...
            <div class="flex-1 min-w-[120px] grid gap-1">
              ...
              <AppSelect v-model="r.branch" class="text-sm">
                <option value="">Global</option>
                <option v-for="b in allBranches" :key="b.uuid" :value="b.uuid">
                  {{ b.name }}
                </option>
              </AppSelect>
            </div>
  ...
  >
    ...
    <div class="grid gap-2 mb-6">
      <p v-if="!roleRows.length" class="text-sm text-muted-foreground italic">
        Este usuario no tiene roles asignados.
      </p>
      <div
        v-for="a in roleRows"
        :key="a.uuid"
        class="flex items-center justify-between p-3 border rounded-md bg-muted/30"
      >
        <div class="flex items-center gap-3">
          <span
            class="text-sm font-bold px-2 py-0.5 rounded-full bg-purple-100 text-purple-700"
            >{{ a.role_detail?.name ?? "—" }}</span
          >
          <span
            v-if="a.organization_detail || a.branch_detail"
            class="text-xs text-muted-foreground"
          >
            {{
              [a.organization_detail?.name, a.branch_detail?.name]
                .filter(Boolean)
                .join(" · ")
            }}
          </span>
        ...
        <div class="flex items-center">
          <button
            class="p-2 rounded-md hover:bg-muted text-primary"
            title="Editar rol"
            :disabled="rolesLoading"
            @click="editRoleAssignment(a)"
          >
            <Pencil :size="16" />
          </button>
          <button
      ...
      <div class="flex gap-2 flex-wrap items-end">
        <div class="flex-1 min-w-[150px] grid gap-1">
          ...
          <AppSelect v-model="newRoleRow.role" class="text-sm">
            <option value="">Seleccione...</option>
            <option v-for="r in allRoles" :key="r.uuid" :value="r.uuid">
              {{ r.name }}
            </option>
          </AppSelect>
        </div>
        <div class="flex-1 min-w-[150px] grid gap-1">
          ...
          <AppSelect v-model="newRoleRow.organization" class="text-sm">
            <option value="">Global</option>
            <option v-for="o in allOrganizations" :key="o.uuid" :value="o.uuid">
              {{ o.name }}
            </option>
          </AppSelect>
        </div>
