...
<template>
  <nav 
    :class="['bg-card border-r border-border h-screen transition-all duration-300 flex flex-col', { 'w-16': collapsed, 'w-64': !collapsed }]"
    @mouseenter="collapsed = false"
    @mouseleave="collapsed = true"
  >

    <!-- HEADER -->
    <div class="h-16 flex items-center justify-center px-4 border-b border-border bg-card">
      <div class="h-8">
        <BrandLogo />
      </div>
    </div>
    ...
    <div class="flex-1 overflow-y-auto py-4">
      <template v-for="group in visibleGroups" :key="group.key">
        ...
        >
          ...
          <ChevronDown
            :size="13"
            :class="['transition-transform duration-200', { 'rotate-180': groupOpen[group.key] }]"
          />
        </button>
        ...
        <template v-if="collapsed || groupOpen[group.key]">
          <RouterLink
            ...
            @click="$emit('navigate')"
          >
            <component :is="item.icon" :size="18" class="shrink-0" :class="item.color" />
    ...
    <div class="h-12 flex items-center justify-center border-t border-border text-muted-foreground">
