...
async function saveProfile() {
  ...
  try {
    ...
    profileError.value =
      d?.message ?? d?.detail ?? "Error al actualizar el perfil.";
  } finally {
  ...
  try {
    ...
    pwdError.value =
      d?.data && typeof d.data === "object"
        ? Object.values(d.data).flat().join(" ")
        : (d?.message ?? d?.detail ?? "Error al cambiar la contraseña.");
  } finally {
...
<template>
  <AppModal title="Configuración de mi cuenta" size="md" @close="emit('close')">
    ...
    <div v-if="activeTab === 'profile'" class="space-y-4">
      <div
        class="grid grid-cols-1 gap-4 p-4 bg-pastel-blue/20 rounded-lg border border-pastel-blue"
      >
        ...
        <div class="flex flex-col">
          <span class="text-[10px] font-bold text-primary uppercase">Email</span
          ><span class="text-sm font-medium">{{
        ...
        <div class="flex flex-col">
          <span class="text-[10px] font-bold text-primary uppercase">Cargo</span
          ><span class="text-sm font-medium">{{
            profile.position || "—"
          }}</span>
        </div>
