  ...
  try {
    ...
    if (JSON.stringify(authStore.menu) !== prevMenu) {
      ...
      if (
        ...
        !allowedPaths.includes(current.path)
      ) {
...
<template>
  <div class="flex h-screen overflow-hidden bg-background relative">
    ...
    <aside
      :class="[
        'fixed inset-y-0 left-0 z-30 transform transition-transform duration-300 md:hidden',
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
      <Topbar @open-sidebar="openSidebar" @open-settings="openSettings" />
